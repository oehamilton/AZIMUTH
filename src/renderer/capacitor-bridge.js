/**
 * Provides window.azimuth (loadData/saveData + stubs) when running in Capacitor (Android).
 * No-op when window.Capacitor is absent (Electron). Dispatches "azimuth-ready" when done.
 */
(function () {
  ...
  var F = window.Capacitor.Plugins && window.Capacitor.Plugins.Filesystem;
  if (F) {
    expose(F);
    return;
  }
  if (typeof import !== "undefined" && import("").then) {
